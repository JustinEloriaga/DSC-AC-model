function r=tvsvar_modified(y,lags,T0,T0B,T0A,T0H,kB,kA,kH,M);
...
for j=1:n-1
    ...
    V(k+count:k+count+j-1,k+count:k+count+j-1,1)=iwishmean(V0A,T0A(j));
    count=count+j;
end
V0H=eye(n)*T0H*kH^2;
% V(k+na+1:end,k+na+1:end,1)=iwishrnd(V0H,T0H);
V(k+na+1:end,k+na+1:end,1)=iwishmean(V0H,T0H);

% drawing the initial H from the prior
H(1,:,1)=mvnrnd_modified(Hbar,eye(n),1);
for t=2:T
    H(t,:,1)=mvnrnd_modified(H(t-1,:,1),V(k+na+1:end,k+na+1:end,1),1);
end

% drawing the initial A from the prior
A(1,:,1)=mvnrnd_modified(Abar,4*(VAbar/2+VAbar'/2),1);
for t=2:T
    A(t,:,1)=mvnrnd_modified(A(t-1,:,1),V(k+1:k+na,k+1:k+na,1),1);
end
...
%     end
    ...
    % simulation smoother
    B(T,:,i)=mvnrnd_modified(shat,sig/2+sig'/2,1);
    for t=T-1:-1:1
        [btTp,StTp]=kback(SHAT(t,:),squeeze(SIG(t,:,:)),squeeze(B(t+1,:,i)),eye(k),squeeze(V(1:k,1:k,i)));
        B(t,:,i)=mvnrnd_modified(btTp,StTp/2+StTp'/2,1);
    end
    ...
    [H(:,:,i+1)]=step_sv_modified(ystar,squeeze(V(k+na+1:end,k+na+1:end,i)),Hbar,squeeze(H(:,:,i)));
    ...
    V(1:k,1:k,i+1)=iwishrnd( (V1B + V1B')/2.0 , T-1+T0B);
    ...
    for j=1:n-1
        ...
        V(k+count:k+count+j-1,k+count:k+count+j-1,i+1)=iwishrnd( (V1A + V1A')/2.0 ,T-1+T0A(j));
        count=count+j;
    end
    ...
    V(k+na+1:end,k+na+1:end,i+1)=iwishrnd( (V1H + V1H)/2.0,T-1+T0H); 
