function r=tvsvar_modified_msv2_gam2(y,lags,T0,T0B,T0A,T0H,kB,kA,kH,M,nreport,nthins);
...
% sig2r initial
% sig2r_old = prior_sig2r.V0 ./ (prior_sig2r.T0 - 1); % start from the prior mean
...
Pt_old = zeros(m,m,T);
for t=1:T
    %Pt_old(:,:,t) = veclAtoC_4v_mex(rt_old(t,:));
...
%         sig2r_old(vvind) = gamrnd(T1, V1^(-1))^(-1);
%     end
...
%             mat_sig2r_rej(sind,vvind) = 0;
        else % reject
            sig2r_old(vvind) = sig2r_old_vvind;
%             mat_sig2r_rej(sind,vvind) = 1;
        end
