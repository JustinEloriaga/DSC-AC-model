function rst = fcst_var_primiceri_msv2_gen(r, hmax, Yest, Yact, Xcalest, info)

% Generalized version of fcst_var_primiceri_msv2.m
...
% Per-variable predictive scores stored in a cell array (generalized)
gb_plike_rao_var = cell(n,1);
for vv = 1:n
    gb_plike_rao_var{vv} = nan(ndraws,hmax);
end
...
for i_d = 1:nsave
    ...
    for i_e = 1:info.nthin
        ...
        for hind = 1:1:hmax
            ...
            % Per-variable predictive scores
            for vv = 1:n
                gb_plike_rao_var{vv}(count,hind) = log(normpdf(Yact(hind,vv), fM(vv), sqrt(fV(vv,vv))));
            end
...
% Per-variable predictive scores
for vv = 1:n
    rst.plike_rao_var_mean{vv}     = mean(exp(gb_plike_rao_var{vv}),1);
    rst.log_plike_rao_var_mean{vv} = mean(gb_plike_rao_var{vv},1);
end

% Backward-compatible field names for the first 3 (or 4) variables
if n >= 1
    rst.plike_rao_11_mean     = mean(exp(gb_plike_rao_var{1}),1);
    rst.log_plike_rao_11_mean = mean(gb_plike_rao_var{1},1);
end
if n >= 2
    rst.plike_rao_22_mean     = mean(exp(gb_plike_rao_var{2}),1);
    rst.log_plike_rao_22_mean = mean(gb_plike_rao_var{2},1);
end
if n >= 3
    rst.plike_rao_33_mean     = mean(exp(gb_plike_rao_var{3}),1);
    rst.log_plike_rao_33_mean = mean(gb_plike_rao_var{3},1);
end
if n >= 4
    rst.plike_rao_44_mean     = mean(exp(gb_plike_rao_var{4}),1);
    rst.log_plike_rao_44_mean = mean(gb_plike_rao_var{4},1);
end
