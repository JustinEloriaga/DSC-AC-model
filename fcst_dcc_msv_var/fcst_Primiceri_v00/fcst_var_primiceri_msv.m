function rst = fcst_var_primiceri_msv(r, hmax, Yest, Yact, Xcalest, info)
...
r.Q = r.Q(:,:,:,nburns+1:end);
r.k = r.k(nburns+1:end,1);
r.d = r.d(nburns+1:end,1);
r.Ainv = r.Ainv(:,:,nburns+1:end);
...
for i_d = 1:nsave
    ...
    for i_e = 1:info.nthin
        ...
        for hind = 1:1:hmax
            ...
            % correlation
            Qmdh = matrix_power(Q0, -d0/2);
            S1 = (1/k0) * Qmdh * A0 * Qmdh';
            Q1 = eye(n) / wishrnd(S1, k0);
            P1 = QtoP(Q1);
