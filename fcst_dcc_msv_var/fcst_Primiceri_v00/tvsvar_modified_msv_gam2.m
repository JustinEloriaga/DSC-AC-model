function r=tvsvar_modified_msv_gam2(y,lags,T0,T0B,T0A,T0H,kB,kA,kH,M,nreport,nthins);
...
% Cogley and Sargent prior

% prior_sig2h.T0 = ones(m,1)*(0.01^2 / 2 );
% prior_sig2h.V0 = ones(m,1)*(1 / 2);

prior_sig2h.T0 = ones(m,1)*(1 / 2); %correction gam2
prior_sig2h.V0 = ones(m,1)*(0.01^2 / 2 ); %correction gam2
...
mat_QT = zeros(m,m,1,nsave);
% mat_Q_rej = zeros(M,T);
mat_Ainv = zeros(m,m,nsave);
mat_d = zeros(nsave,1);
% mat_d_rej = zeros(M,1);
mat_k = zeros(nsave,1);
% mat_k_rej = zeros(M,1);
% mat_sig2h_rej = zeros(M, m);
...
%             mat_sig2h_rej(sind,vvind) = 0;
        else % reject
            sig2h_old(vvind) = sig2h_old_vvind;
%             mat_sig2h_rej(sind,vvind) = 1;
        end
    ...
    if rem(sind,nthins) == 0
        ...
        mat_QT(:,:,:,saveind) = Qt_old(:,:,end);
        mat_Ainv(:,:,saveind) = Ainv_old;
        mat_d(saveind,1)      = d_old;
        mat_k(saveind,1)      = k_old;
