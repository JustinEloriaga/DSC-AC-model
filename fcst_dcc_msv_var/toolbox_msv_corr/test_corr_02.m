...
% Correlation process
n_r=m*(m-1)/2;
sig2r = 0.01*ones(n_r,1);
mr0 = ones(n_r,1);
Vr0_scale = 2^2*ones(n_r,1);
Vr0 = sig2r.*Vr0_scale; % for covenience
r0 = mr0 + diag(sqrt(Vr0))*randn(n_r,1);

rt_old = zeros(T,n_r);
for t=1:T
    r0 = r0 + diag(sqrt(sig2r))*randn(n_r,1);
    rt_old(t,:) = r0;
end
rt = rt_old;
...
%         Pt_old(:,:,t) = veclAtoC(rt_old(t,:));
    ...
    if rem(sind,1) == 0
        ...
        disp(['time per draw = ', num2str(time_per_draw)]);
    end
end
