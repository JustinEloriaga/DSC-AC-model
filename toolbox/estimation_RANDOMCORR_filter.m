function estimation_RANDOMCORR_filter(info)
% TVP-VAR with Hansen's MSV2 model for MARS portfolio data
% FILTER-ONLY VARIANT: B drawn from filtered marginals, no smoother.
...
r = tvsvar_modified_msv2_gam2_gen_filteronly(Yest, info.p, info.T0, info.kB, M, info.nreport, info.nthin);

%% Save
strname      = strrep(Xcalest{end}, '/', '-');
savefilename = ['RANDOMCORR_FILTER_', info.freq, '_', strname, '.mat'];
